        ...
        <div v-if="isAuthorised" class="internaPage fullheight overflow">
            ...
            <div v-for="p in subpage" :key="p.id"  class="zone">
              <h1 class="title">{{p.title}}</h1>
                  ...
                  <div class="image" v-if="zone.__typename === 'ComponentLayoutsImage'">
                  ...
                  <div class="video" v-if="zone.__typename === 'ComponentLayoutsVideo'">
                     ...
                     <Vimeo v-if="zone.platform==='vimeo'" :id="zone.videoId"/>
                     <Youtube v-if="zone.platform==='youtube'" :id="zone.videoId"/>
                  </div>  
...
export default {
  ...
  watch: {
    subpage: function(){
     ...
     // this.$passwordProtect.newPass(this.pass, this.slug)

      this.isAuthorised = this.$passwordProtect.isAuthorised(this.slug, this.pass)
  ...
  methods: {
    loginUser(e) {
      e.preventDefault()
      this.$passwordProtect.authorise(this.methodLoginValue, this.pass, this.slug)
      this.isAuthorised = this.$passwordProtect.isAuthorised( this.slug, this.pass)
